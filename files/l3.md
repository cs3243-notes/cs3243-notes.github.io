...
$~~~~~~~~~~ \hat{f}\_{pop}(S\_{k+1}) = g(S\_{k}) + c(S\_k,S\_{k+1}) + {h}(S\_{k+1}) ~~~~ \text{from: }\hat{g}\_{pop}(S\_{k+1}) = g(S\_{k+1})$
...
$~~~~~~~~~~$ $\hat{f}\_{pop}(S\_{k+1}) \leq g(S_k)+ c(S_k,S\_{k+1})+ {h}(S\_{k+1}) ~~~~~~~~~~~~ \text{from UCS proof } \forall_{i\in {1,\ldots,u}} (g_{pop}(S_i)=g(S_i))$
