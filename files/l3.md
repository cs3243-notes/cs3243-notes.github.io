...
**Property 1** $$\hat{f}_{pop}(S_0) \leq \hat{f}_{pop}(S_1),\leq,\ldots,\hat{f}_{pop}(S_k) \leq \hat{f}_{pop}(S_u)$$

**Property 2** $$ \hat{f}\_{pop}(S_i) = f(S_i)$$
...
The property $ h(S*{i}) \leq c(S*{i},S*{i+1}) + h(S*{i+1})$ is known as **consistency**.

$h(S_{i})  \leq c(S_{i},S_{i+1}) + h(S_{i+1})$ \
$h(S_{i})  \leq c(S_{i},S_{i+1}) + c(S_{i+1},S_{i+2}) + h(S_{i+2})$\
$~~~~~$ ..\
$~~~~~$ ..\
$ h(S*{i}) \leq c(S*{i},S*{i+1}) + c(S*{i+1},S*{i+2}) + \ldots + c(S*{k},S*{u}) +h(S*{u}) $
...
$~~~~~$ $~~~~~$ $\forall{i \in \{1,\ldots,u\}} \; f(S_i) = g(S_i)+h(S_i)$
...
$~~~~~$ $~~~~~$ $\hat{f}_{pop}(S_{k+1}) = g(S_{k}) + c(S_k,S_{k+1}) + {h}(S_{k+1})$\
$~~~~~$ $~~~~~$ $~~~~~$ $~~~~~$ $\text{from: }\hat{g}_{pop}(S_{k+1}) = g(S_{k+1})$\
$~~~~~$ $~~~~~$ $\hat{f}_{pop}(S_{k+1}) \geq g(S_{k}) + h(S_{k})$ $~~~~~$ $~~~~~$ $~~~~~$ $ \text{from equation 5}$\
$~~~~~$ $~~~~~$ $\hat{f}_{pop}(S_{k+1}) \geq \hat{f}_{S_{k}} $ $~~~~~$ $~~~~~$ $~~~~~$ $~~~~~$ $~~~~~$ $~~~~~~$ $\text{from equation 7}$
...
$~~~~~$ $~~~~~$ $\hat{f}_{pop}(S_{k+1}) \leq f(S_{k+1})$ $~~~~~$ $~~~~~$ $~~~~~$ $~~~~~$ $~~~~$ $\text{from equation 5}$
...
$$d(S_i, u) \leq d(S_i, S_{i+1}) + d(S_{i+1}, u)  $$
$$d(S*i, u) \leq c(S_i, S*{i+1}) + d(S*{i+1}, u)$$
$$h(S_i) \leq c(S_i, S*{i+1}) + h(S\_{i+1}) $$
...
\caption{Example to design admissible heuristic function; Agent A wants to reach to agent G.\label{fig:gird}.}
...
\caption{Example to discuss Greedy-Best-First-Search; Roadmap from Home to School.\label{fig:gbfs}.}
